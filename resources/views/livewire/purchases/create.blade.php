    ...
    @if ($showQuickProductModal)
        <div class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-6">
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl max-w-2xl w-full p-5 sm:p-8 shadow-2xl space-y-4 sm:space-y-6 text-slate-900 dark:text-white max-h-[92vh] flex flex-col justify-between overflow-hidden">
                ...
                <div class="space-y-4 overflow-y-auto pr-1 text-sm min-h-[360px] pb-28">
                    ...
                    <div class="border-t border-slate-200 dark:border-slate-700 pt-3 space-y-2.5">
                        <div class="flex justify-between items-center">
                            <label class="font-extrabold text-amber-600 dark:text-amber-400 text-xs">🔄 Satuan Konversi Tambahan (Misal: Batang / Dus / Ikat)</label>
                            <button type="button" wire:click="addQuickAdditionalUnitRow" class="bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 text-slate-900 dark:text-white font-bold px-3 py-1 rounded-lg text-xs cursor-pointer transition">
                                + Tambah Satuan Lain
                            </button>
                        </div>

                        @forelse ($quickAdditionalUnits as $idx => $row)
                            <div class="bg-slate-100 dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-700 grid grid-cols-12 gap-2 items-center">
                                <div class="col-span-12 sm:col-span-4 relative" x-data="{ openUnitRow: false }" :class="{ 'z-[999]': openUnitRow }">
                                    <label class="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-0.5">Satuan</label>
                                    <button 
                                        type="button" 
                                        @click="openUnitRow = !openUnitRow" 
                                        class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white rounded-lg p-2 text-xs font-bold focus:border-amber-500 outline-none flex justify-between items-center cursor-pointer shadow-sm"
                                    >
                                    ...
                                    <div 
                                        ...
                                        @click.outside="openUnitRow = false" 
                                        x-transition
                                        class="absolute z-[9999] left-0 right-0 bottom-full mb-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-48 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-700/50"
                                        style="display: none;"
                                    >
                                        @foreach ($units as $u)
                                            <div 
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    <label class="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-0.5">Faktor Konversi (x Base)</label>
                                    <input type="number" step="0.01" wire:model="quickAdditionalUnits.{{ $idx }}.conversion_factor" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-amber-600 dark:text-amber-400 font-mono font-bold rounded-lg p-2 text-xs outline-none">
                                </div>
                                <div class="col-span-5 sm:col-span-3">
                                    <label class="block text-[11px] font-semibold text-slate-500 dark:text-slate-400 mb-0.5">Harga Jual</label>
                                    <div class="relative flex items-center">
                                        <span class="absolute left-2 text-amber-600 dark:text-amber-400 font-mono text-xs font-bold select-none">Rp</span>
                                        <input type="number" wire:model="quickAdditionalUnits.{{ $idx }}.selling_price" class="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-emerald-600 dark:text-emerald-400 font-mono font-bold rounded-lg p-2 pl-7 pr-6 text-xs outline-none">
                                    </div>
                                </div>
                                <div class="col-span-1 text-center">
                                    <button type="button" wire:click="removeQuickAdditionalUnitRow({{ $idx }})" class="text-red-500 hover:text-red-700 font-bold text-base cursor-pointer">&times;</button>
                                </div>
                            </div>
                        @empty
                            <p class="text-xs text-slate-500 italic">Belum ada satuan konversi tambahan.</p>
                        @endforelse
                    </div>
                </div>
