<div class="min-h-screen bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-100 p-6 space-y-6 font-sans transition-colors duration-200">
    ...
    <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-xl flex justify-between items-center transition-colors">
        <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white">Master Data Pelanggan & Piutang</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Daftar pelanggan, toko langganan, kontraktor, dan pengaturan limit piutang</p>
        </div>
            ...
            <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
                <thead class="bg-slate-100 dark:bg-slate-900/80 uppercase text-xs font-bold text-slate-500 dark:text-slate-400">
                    <tr>
                        ...
                        <th class="p-3.5 text-right">Limit Kredit Piutang</th>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700/60">
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                            ...
                            <td class="p-3.5 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="editCustomer({{ $c->id }})" class="bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 px-3 py-1.5 rounded-lg text-xs font-bold transition">
                                        ✏️ Edit
                                    </button>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="p-8 text-center text-slate-400 text-base">Belum ada pelanggan tersimpan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl max-w-xl lg:max-w-2xl w-full p-8 shadow-2xl space-y-6 text-slate-900 dark:text-white max-h-[92vh] flex flex-col justify-between overflow-hidden">
                ...
                <div class="space-y-5 text-base overflow-y-auto pr-1">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <x-ui.input label="Limit Piutang (Rp)" type="number" wire:model="creditLimit" placeholder="0" />
                        <x-ui.input label="Jatuh Tempo (Hari)" type="number" wire:model="paymentTermsDays" placeholder="14" />
                    </div>
                    <div>
        ...
        <div class="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 sm:p-6">
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl max-w-xl lg:max-w-2xl w-full p-8 shadow-2xl space-y-6 text-slate-900 dark:text-white max-h-[92vh] flex flex-col justify-between overflow-hidden">
                ...
                <div class="space-y-5 text-base overflow-y-auto pr-1">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <x-ui.input label="Limit Piutang (Rp)" type="number" wire:model="creditLimit" />
                        <x-ui.input label="Jatuh Tempo (Hari)" type="number" wire:model="paymentTermsDays" />
                    </div>
                    <div>
                        <label class="block text-sm font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1">Alamat</label>
